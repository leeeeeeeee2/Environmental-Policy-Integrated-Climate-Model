...
!     MANUAL OPTION.  EROSION AND RUNOFF ARE ESTIMATED.
    ...
    5 NII=0
      ...
      ELSE
          IF(QXM>0.)THEN
              QPX=QXM/24.
              IF(IRR/=1.AND.RHTT>0..AND.RGIN>0.)THEN
                  X1=1000.*RGIN/RHTT
      ...
      VAR(60)=VAR(60)+AFN
      IF(NOP>0.AND.AFN>0.)WRITE(KW(1),133)IYR,MO,KDA,AFN,AIR,XHSM
   10 JRT=0
      IF(IRR==5)THEN
	      AWC(JJK)=AWC(JJK)+AIR
	      AIR=0.
	  END IF
      RETURN
    9 FORMAT(1X,3I4,2X,'IRRIGATE',10X,'VOL=',F5.0,'MM',2X,'WS=',F6.2,&
      2X,'WTN=',E12.4,'kpa',2X,'PWDF=',F6.0,'MM',2X,'TRGR=',F7.2,&
      2X,'Q=',F5.0,'MM',2X,'Y=',F5.1,'t/ha',2X,'HUSC=',F6.2/2X,'COST=',&
      F7.2,'$/ha')
