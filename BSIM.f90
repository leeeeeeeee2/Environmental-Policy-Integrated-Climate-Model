...
!     NUMBER OF YEARS                                                                
      ...
      DIMENSION KDT(12,12),KTP(7),MNST(7),JGO(MNC)                                            
      ...
      DATA MNST/1,2,3,4,5,6,7/,JBG,KTF/2*0/                                                
...
!     IF(KFL(6)>0)WRITE(KW(6),477)(HED(KA(I)),I=1,NKA)                               
      ...
      ' YLN','CPNM',' YLD','TOTN'                                                    
      IF(KFL(27)>0)WRITE(KW(27),516)(HED(KD(K)),K=1,NKD),(HEDS(KS(K)),K=1,NKS)                                       
...
!     BEGIN DAILY SIMULATION.                                                        
      ...
      PBWM=PMAV(FMO1,FMO2,PBW(LW,I2,I1),PBW(LW,I3,MOP))                              
      ...
      BCV=BCV*.85                                                                    
      IF(NGN>0)THEN
          ! READ WEATHER VARIABLES INCLUDED IN NGN
          IF(IGSD==0.OR.IY/=IGSD)THEN
              DO
                  ...
                  READ(KR(7),103,IOSTAT=NFL)SRAD,TMX,TMN,RFV,RHD,U10,X1,EVI,REP
                  IF(NFL==0.AND.(IDA<=NSTP.OR.NSTP==0))THEN
                  ...
                  IF(RFV<0.)THEN                                                                 
                      CALL WRWD(1)
                  ELSE
                      IF(RFV>900.)CALL WRWD(0)
                  END IF        
                  RFV=RFV*RNCF(MO)
                  IF(KGN(2)==0.OR.TMX<=TMN)THEN
                      CALL WGN                                                                       
                      CALL WTAIR
                      X1=TMX-TMN                                                                     
                      TMX=TMX+TMXF(MO)                                                               
                  ...
                  IF(SRAD<1.E-5.OR.SRAD>900..OR.KGN(3)==0)THEN                                                  
                      ...
                      CALL WSOLRA                                                                  
                  END IF
                  IF(RHD<0..OR.RHD>1..OR.IRH>0.AND.RHD<900.)THEN
                      ! RHD=DEW POINT TEMP--CONVERT TO RELATIVE HUM
                      TX=.5*(TMX+TMN) 
                      RHD=MIN(.99,ASVP(RHD+273.)/ASVP(TX+273.))
                      IRH=1                                                                    
                  ELSE
                      IF(RHD<1.E-5.OR.RHD>900..OR.KGN(5)==0)THEN 
                          ! RHD IS MISSING DATA (0. OR 999.) OR NOT INCLUDED IN NGN                                                
                          IF(III==0)CALL WGN                                                           
                          CALL WRLHUM                                                                  
                      END IF                                                                         
                  END IF    
                  IF(U10<1.E-5.OR.U10>900..OR.KGN(4)==0)CALL WNSPD
                  EXIT
              END DO                                                     
          END IF
      ELSE    
    ...
    !     COMPUTE SEDIMENT YLD                                                           
      ...
      X1=RFTT*GWST
      RSSF=X1*RFPK
      SMM(110,MO)=SMM(110,MO)+RSSF
      VAR(110)=RSSF
      DPRK=X1-RSSF
      SMM(111,MO)=SMM(111,MO)+DPRK
      VAR(111)=DPRK
      CONC=GWSN/(GWST+1.E-10)
      TNL=CONC*X1
	  CNV=TNL/(RSSF*PRMT(84)+DPRK)
	  CNH=CNV*PRMT(84)
      RSFN=CNH*RSSF
      SMM(112,MO)=SMM(112,MO)+RSFN
      VAR(112)=RSFN
      DPKN=CNV*DPRK
      SMM(113,MO)=SMM(113,MO)+DPKN
      VAR(113)=DPKN
      GWSN=MAX(1.E-10,GWSN-RSFN-DPKN)
      DO K=1,NDP
          X3=GWPS(K)/GWST
          RSPS(K)=MIN(GWPS(K),RSSF*X3)
          GWPS(K)=GWPS(K)-RSPS(K)
          VARP(12,K)=MIN(GWPS(K),DPRK*X3)
          GWPS(K)=GWPS(K)-VARP(12,K)
          VARP(11,K)=RSPS(K)
          SMMP(11,K,MO)=SMMP(11,K,MO)+RSPS(K)
          SMMP(12,K,MO)=SMMP(12,K,MO)+VARP(12,K)
      END DO
      GWST=MAX(1.E-10,GWST-DPRK-RSSF)
      CALL HEVP                                                                      
      ...
      443 IF(KG(JJK)==0.AND.JPL(JJK)==0)THEN                                             
          ...
          IF(DM(JJK)/(DM1(JJK)+1.E-5)<.99.AND.RWPC(1,JJK)<RWPC(2,JJK))GO TO 590                                                     
          IF(IDC(JJK)==NDC(7).OR.IDC(JJK)==NDC(8).OR.IDC(JJK)==NDC(10))GO TO 25                                                        
          IF(YLAT>0.)THEN
              IF(MO==12)GO TO 121
          ELSE
              IF((MO==7.AND.TBSC(JJK)>7.).OR.(MO==12.AND.KDA==30))GO TO 121                                                                               
          END IF    
          GO TO 25
   ...
   30 IF(LUN/=35)CALL NPCY
      ...
      DO J=1,NBSL
          ...
          IF(STMP(ISL)>0.)THEN
              Z5=500.*(Z(ISL)+XX)
              IF(ICP==0)THEN
                  CALL NCNMI_PHOENIX(Z5,CS,EAR(ISL))
              ELSE    
                  CALL NCNMI_CENTURY(Z5,CS)
              END IF
              IF(IDN<3)THEN
                  DZA=1000.*(Z(ISL)-XX)
	              IF(IDN==2)THEN
      ...
      IF(IGO>0)THEN
          ...
          DO IN2=1,IGO                                                                    
	          ...
	          IF(JPL(JJK)>0)THEN
	              ...
	              IF(PDSW/FCSW<PRMT(11).OR.(HU(JJK)<GMHU(JJK).AND.MO<12))CYCLE
                  JPL(JJK)=0                                                                     
              ...
              IF(KFL(12)>0)THEN
                  ...
                  IF(NSTP>0)II=IRLX 
                  X2=1000.*YLD1(N1,JJK)                                                                                                                                          
                  WRITE(KW(12),513)IYR,MO,KDA,II,CPNM(JJK),HUI(JJK),AJHI(JJK),&
                  SLAI(JJK),DM(JJK),RW(JJK),STL(JJK),HIX,YLDX,X2,UN1(JJK),&
                  VAR(99),VAR(100),(SF(J,N1,JJK),J=1,7),(RWT(LID(K),JJK),K=1,NBSL)                                              
              END IF            
          END DO
      END IF
      CALL SCONT(0)                                                                          
...
! 	  SCI=SCI+X1-RFV+QD+PKRZ(LNS)+SST                                                    
  ...
  200 CALL NEVN                                                                      
      ...
      ZNH3,ZNO3,WNO3(LD1),PKRZ(LD1),VNO3(LD1),ALB,HUI(JJK),AJHI(JJK),&
      SLAI(JJK),DM(JJK),RW(JJK),STL(JJK),HIX,YLDX,X2,UN1(JJK),VAR(99),&
      VAR(100),(VARS(K),K=23,28)
      IF(KFL(27)>0)WRITE(KW(27),91)IYR,MO,KDA,(VAR(KD(K)),K=1,NKD),&
      (VARS(KS(K)),K=1,NKS)                              
	  CALL SWN1530                                                                        
...
!     WRITE MO VALUES AND SUM YEARLY VALUES                                          
    ...
    !         PRINTOUT CROP MONTHLY                                                        
          ...
          DO J1=1,N1                                                                 
	          ...
	          IF(ETG(J1,J)>.1)THEN                                                                
	              XTP(3,J1,J)=MIN(20.,1000.*(YLD1(J1,J)+YLD2(J1,J))/ETG(J1,J))                               
	          ELSE                                                                                
	              XTP(3,J1,J)=0.                                                                    
	          END IF                                                                              
              XX=NPSF(J1,J)                                                                  
              ...
              IF(IY==IPY)THEN                                                                
                  ...
                  ELSE
                      ...
                      X1=1000.*YLCF(J1,J)
                  END IF
                  WRITE(KW(1),106)CPNM(J),YLD1(J1,J),YLD2(J1,J),DM1(J),&
  ...
  555 IF(KFL(23)>0)THEN                                                              
      ...
      DO K=1,LC                                                                       
          ...
          ELSE
              IF(IDC(J)/=NDC(1).AND.IDC(J)/=NDC(4))DM1(J)=0.
          END IF
      END DO
...
!     SMY(68)   = SOIL WATER IN TOP 10 mm OF SOIL (mm)                               
  ...
  499 WB1=SMY(34)                                                                    
   ...
   84 CONTINUE                                                                       
      ...
      V3=AUNIF(IDG(3))                                                                                                                                               
      END IF
      IF(ISW<=3.OR.ISW==7.OR.ISW==9)THEN
          XX=0.                                                                          
          K=1                                                                            
          TPAW=0.                                                                        
	      DO I=1,NBSL                                                                
              ...
              SELECT CASE(ISW+1)
                  ...
                  CASE(10)         
                      CALL SWRTNNW(CLA(J),SIL(J),SAN(J),Y1,BD(J),X2,X3)
              END SELECT                                                                     
              XY=1.-ROK(J)*.01                                                               
              XZ=XY*X1                                                                       
              S15(J)=X2*XZ                                                                   
...
!         CALL SCONT(1)                                                                   
   ...
   89 FORMAT(1X,3I4,2X,'GERMINATION--0.2 m SW = ',F7.1,'mm',2X,'HU = ',&
      F4.0,'c',2X,'HUSC = ',F6.2,2X,A4)
   90 FORMAT(1X,3I4,2X,'LIME',2X,'RATE=',F6.0,'t/ha',1X,'HUSC=',F6.2,2X,&
      'COST=',F7.0,'$/ha')
  ...
  106 FORMAT(2X,A4,1X,'YLD=',F6.1,'/',F6.1,2X,'BIOM=',F6.1,'t/ha',2X,&               
      ...
      F5.0,2X,'CAW=',F6.0,'mm',2X,'GSET=',F6.0,'mm',2X,'WUEF=',F10.2,&
  ...
  516 FORMAT(4X,'Y   M   D  ',140(6X,A4,2X))                                                 
  ...
  526 FORMAT(4X,'Y M RT#  CPNM',6X,'WS',6X,'NS',6X,'PS',6X,'KS',6X,'TS',&            
      6X,'AS',6X,'SS',6(4X,A4))                                                      
  527 FORMAT(4X,'Y   M   D  RT#',4X,'CPNM',6X,' HUI',6X,'AJHI',6X,' LAI',&
      6X,'BIOM',6X,'  RW',6X,' STL',6X,'  HI',6X,'YLDX',6X,'YLDF',6X,&
      'UNO3',6X,' NPP',6X,' NEE'8X,'WS',8X,'NS',8X,'PS',8X,'KS',8X,'TS',&
      8X,'AS',8X,'SS',7X,'RW1',7X,'RW2',7X,'RW3',7X,'RW4',7X,'RW5',7X,&
      'RW6',7X,'RW7',7X,'RW8',7X,'RW9',6X,'RW10',6X,'RW11',6X,'RW12',6X,&
      'RW13',6X,'RW14',6X,'RW15')                                                     
