...
!     HARVESTING, AND AUTOMATIC FERTILIZER APPLICATIONS AT PLANTING.
   ...
   27 IF(KG(I2)>0)GO TO 26
    ...
    6 EE=EMX(JT1)
	  ...
	  DMLX(JJK)=XLAI(JJK)
      DMX=TLD(JT1)
	  IF(II/=NHC(19).AND.II/=NHC(2))CALL TMIX(EE,DMX,0,0)
   ...
   10 DO K=1,NN
      ...
      END IF
      HVWC=HWC(IRO,KT)                                                                 
      IF(HVWC>0..AND.WCYD>HVWC.AND.MO<12)THEN
          JRT=1
          KOMP(KT)=0
          RETURN
      END IF
   ...
   45 XZ=X1*STL(JJK)
      ...
      UP1(JJK)=UP1(JJK)-X5
      DM(JJK)=DM(JJK)-XZ
      RW(JJK)=DM(JJK)-STL(JJK)
      RTO=RW(JJK)/RWL
      DO J=1,LRD
          ISL=LID(J)
          RWT(ISL,JJK)=RWT(ISL,JJK)*RTO
      END DO
      X3=MAX(0.,RW(JJK))
      RWF(N1,JJK)=X3
   ...
   25 TYN=TYN+YLN
      ...
      CPNM(JD),YY,YLSD,AGPM,DM1(JJK),X3,X1,X6,X7,WCYD,XHSM,CSTX
      GO TO 6
   26 IF(NOP>0.AND.II==NHC(19))WRITE(KW(1),62)IYR,MO,KDA,TIL(JT1),&
      ...
      RETURN
   28 FORMAT(1X,3I4,2X,A8,2X,'DPTH = ',F5.0,'mm',2X,'HUSC = ',F6.2,2X&
      ,'COST=',F7.0,'$/ha')
   29 FORMAT(1X,3I4,2X,A8,2X,A4,2X,'YLD=',F7.2,'t/ha',2X,'YLSD=',F7.2,&
      't/ha'2X,'AGPM=',F7.2,'t/ha',2X,'BIOM=',F7.2,'t/ha',2X,'RW=',F7.2,&
      't/ha',2X,'HI=',F6.2,2X,'PSTF=',F5.2,2X,'NCN=',F6.3,'G/G',2X,'WCY=',&
      F5.2,2X,'HUSC=',F5.2,2X,'COST=',F7.0,'$/ha')
