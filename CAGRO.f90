...
!     ACTIVE STRESS CONSTRAINT.
      ...
      ELSE
          RF=MIN(RF,.99)
      END IF
      RW(JJK)=PRMT(92)*RWL+(1.-PRMT(92))*RF*XX        
      DRW=RW(JJK)-RWL
      DM(JJK)=XX
      ...
      IF(IDC(JJK)==NDC(7).OR.IDC(JJK)==NDC(8).OR.IDC(JJK)==NDC(10))THEN
          X1=100.*HSM/AHSM
          IF(X1>75.)THEN
              STL0(JJK)=MAX(STL0(JJK),STL(JJK))
              F=X1/(X1+EXP(SCRP(29,1)-SCRP(29,2)*X1))
              FF=F-FLF0(JJK)
              FALF=STL0(JJK)*FTO(JJK)*FF
              !ADFL=ADFL+FALF
              FLF0(JJK)=F
	          SMM(88,MO)=SMM(88,MO)+FALF
	          !SLAI(JJK)=MAX(.001,SLAI(JJK)-FALF)
              DM(JJK)=DM(JJK)-FALF
              ...
              UP1(JJK)=UP1(JJK)-XY
              !WRITE(KW(1),'(1X,A,3I4,5F10.4)')'~~~~~',IYR,MO,KDA,F,FF,STL0(JJK),FALF,ADFL
          END IF
      END IF
