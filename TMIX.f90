...
!     INTERVAL.
      USE PARM
      DIMENSION TST(58),DUM(15),XTMP(15),XTP(8),YTP(8)
      DATA ISM/58/
      AD1=0.
      ZLSC1=0.
      ZLMC1=0.
          ...
          IF(NMIX==0)THEN
              ...
              !     MOW, SHRED, ETC
                  DO J=1,LC
                      ...
                      IF(CPHT(J)+DMX<0..OR.STL(J)<.001)CYCLE
                      XX=(CPHT(J)+DMX)/CPHT(J)
                      ZZ=XX*STD(J)
                      ZO=XX*STDO
                      ZL=XX*STL(J)
                      STD(J)=STD(J)-ZZ
      ...
      DO J=1,J1
          ...
          RX=MIN(1.,(100.-ROK(LL))/(100.-XTMP(LL)))
          FC(LL)=FC(LL)*RX
          S15(LL)=S15(LL)*RX
          PO(LL)=PO(LL)*RX
          CALL SPOFC(LL)
...
!     WHSC(ISL)=WHSC(ISL)+TST(8)*XX
      ...
      END IF
      IF(XTMP(ISL)>0.)THEN
          RX=MIN(1.,(100.-ROK(ISL))/(100.-XTMP(ISL)))
          FC(ISL)=FC(ISL)*RX
          S15(ISL)=S15(ISL)*RX
          PO(ISL)=PO(ISL)*RX
          CALL SPOFC(ISL)
