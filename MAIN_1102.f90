...
!     EPIC1102 COMPLETE 20110201                                                     
      ...
      CHARACTER(2)::RFPT                                                               
      CHARACTER(4)::ANM,AYR                                                            
      CHARACTER(8)::AGIX,FMT
      CHARACTER(80)::A1,A2,ADUM,AGSM,CM,FCMOD,FCROP,FFERT,FMLRN,FOPSC,&
      FPARM,FPEST,FPRNT,FSITE,FSOIL,FTILL,FTR55,FWIDX,FWIND,FWLST,FWPM1,&
      FWPM5,SOLS,TITW5,WINDFILE,WPM1FILE                                                              
      INTEGER*8 ISIT,INPS,IOPS                                                  
      REAL*8 PPX                                                                     
      DIMENSION AYR(150),CM(9),ASG(4),TITW5(2)                                             
      ...
      DO I=1,SIZE(KW)                                                                          
   	      KW(I)=I+50                                                                     
	  END DO
      OPEN(KW(MSO+6),FILE='WORKSPACE.DAT')
      ADUM='EPICERR.DAT'
      CALL OPENV(KW(MSO),ADUM,0,KW(MSO))
...
!          > 0 RUNS BATCH MODE FROM RUNALL.BAT                                   	  
      ...
      END IF
      ADUM='EPICFILE.DAT'
      CALL OPENV(KR(12),ADUM,0,KW(MSO))
      ADUM='EPICCONT.DAT'                                            
      CALL OPENV(KR(21),ADUM,0,KW(MSO))                                            
      ADUM='AYEAR.DAT'
      CALL OPENV(KR(28),ADUM,0,KW(MSO))
...
!            PLANTING)
! 15  IRRS = 0 FOR ROOT RESPIRATION IN NDNITCI
!          > 0 NO ROOT RESPIRATION IN NDNITCI                                                               
...
!          = 2 FOR INPUTTING CO2                                                     
! 29  NTV  = 0 FOR ORIGINAL EPIC NITVOL EQS                                          
!          > 0 FOR REVISED NITVOL EQS                                   
...
!          = 1 FOR ARMEN KEMANIAN CARBON/CLAY FUNCTION
!          = 2 FOR NEW O2 RATIO METHOD
...
! 40  ICP  = 0 FOR NCNMI_PHOENIX
!          > 0 FOR NCNMI_CENTURY
!     LINE 1/2                                                                       
      READ(KR(21),300)NBYR,IYR0,IMO0,IDA0,IPD,NGN,IGN,IGS0,LPYR,IET,ISCN&            
      ,ITYP,ISTA,IHUS,IRRS,NVCN,INFL0,MASP,LBP,NSTP,IGMX,IERT,ICG,LMS,&              
      ICF,ISW,IRW,ICO2,NTV,ICOR,IDN,NUPC,IOX,IDI0,ISAT,IAZM,IPAT,IEVI,&
      IPRK,ICP
      IF(IDN==0)IDN=2                                        
      MYR=NBYR                                                                       
      SELECT CASE(IDI0+1)
          ...
          CASE(3)
              ...
              END DO
      END SELECT                              
      CALL OPENV(KR(6),FMLRN,0,KW(MSO))                                                           
      ...
      ELSE
          ADUM='EPICRUN.DAT'
          CALL OPENV(KR(11),ADUM,0,KW(MSO))                                             
      END IF		                                                      
...
!            (0=MUST;   2=USLE;  3=MUSS;  4=MUSL;  
!             6=RUSLE;  7=RUSL2)
! 35  RST0 = BASE STOCKING RATE (ha/hd)                                                     
! 36  STF0 = FRACTION OF STORAGE INTERACTING WITH NO3 LEACHING                       
!     LINE 3/6                                                                       
      ...
      PEC0,VLGN,COWW,DDLG,SOLQ,GZLM,FFED,DZ,DRV,RST0,STF0
      ANG=ANG0/CLT
	  IF(DZ<1.E-10)DZ=.1                                                                        
!     READ ECONOMIC DATA                                                             
...
!     LINE 7                                                                         
      ...
      END DO                                                                         
      IF(ISW==7.OR.ISW==8)THEN 
          ADUM='SOIL35K.DAT'                                                                 
	      CALL OPENV(KR(29),ADUM,IDIR(1),KW(MSO))                                          
...
!	  CALL DATE_AND_TIME(AYMD,AHMS)                                                      
      ADUM='RTSOIL.DAT'
      IF(NSTP>0)CALL OPENV(KW(MSO+3),ADUM,0,KW(MSO))                                         
...
!     LINE 15/16                                                                     
      ...
      IF(KFL(MSO+1)/=0)THEN
          ADUM='RUN1102.SUM'
          CALL OPENV(KW(MSO+1),ADUM,0,KW(MSO))                                                                                                    
...
!     LINE 1/2                                                                       
      ...
      READ(KR(24),'()')
      CALL APAGE(0)                                                                  
      WRITE(KW(1),'(//1X,A,A/)')'____________________WEATHER DATA_________',&            
      '_______________'                                                               
	  IYX=IYR0-1880                                                                       
      IF(ICO2==0)THEN
...
! 10  PBW(1) = MONTHLY PROBABILITY OF WET DAY AFTER DRY DAY (BLANK IF                
!              UNKNOWN OR IF DAILY PRECIPITATION IS INPUT)                           
! 11  PBW(2) = MONTHLY PROBABILITY OF WET DAY AFTER WET DAY (BLANK IF                
!              UNKNOWN OR IF DAILY PRECIPITATION IS INPUT)                           
! 12  UAVM   = AV NO DAYS OF PRECIPITATION/MO (BLANK IF PRECIP IS                    
!              GENERATED AND IF PBW 1&2 ARE INPUT)                                   
...
!     LINES 3/15                                                                     
      IF(IW==1)THEN
          ...
          READ(KR(24),310)(PBW(1,IW,I),I=1,12)                                           
          READ(KR(24),310)(PBW(2,IW,I),I=1,12)                                           
	  ...
	  ELSE                                                                              
          ...
          READ(KR(18),310)(PBW(1,IW,I),I=1,12)                                           
          READ(KR(18),310)(PBW(2,IW,I),I=1,12)                                           
      ...
      DO I=1,12                                                                   
	      ...
	      END IF                                                                              
          IF(PBW(1,IW,I)>0.)THEN                                                         
              UAVM(I)=XM*PBW(1,IW,I)/(1.-PBW(2,IW,I)+PBW(1,IW,I))                          
          ELSE                                                                           
              PBW(1,IW,I)=BTA*(UAVM(I)+.0001)/XM                                           
              PBW(2,IW,I)=1.-BTA+PBW(1,IW,I)                                               
          END IF                                                                         
      ...
      WRITE(KW(1),243)HED(4),(RMO(IW,I),I=1,12),AAP,HED(4)                        
      WRITE(KW(1),223)'SDRF',(RST(2,IW,I),I=1,12),'SDRF'                             
      WRITE(KW(1),224)'SKRF',(RST(3,IW,I),I=1,12),'SKRF'                             
      WRITE(KW(1),225)'PW/D',(PBW(1,IW,I),I=1,12),'PW/D'                             
      WRITE(KW(1),225)'PW/W',(PBW(2,IW,I),I=1,12),'PW/W'                             
      ...
      ELSE   
          ...
          DO WHILE(II/=IWND)                                                                             
	          ...
	          IF(NFL/=0)THEN
                  ...
                  ELSE
	                  WRITE(KW(MSO),'(A,A8,A,I4,A)')' !!!!! ',ASTN,&
	                  ' WIND NO = ',IWND,' NOT IN MO WIND LIST FILE'
	                  PAUSE
                  END IF	                  
	          END IF                                                                               
	      END DO         
...
!     LINES 4/19                                                                     
      ...
      SELECT CASE(IET)                                                               
          ...
          CASE(3)                                                                      
              ...
              ' TO EST POT ET**********'                                               
          CASE(4)
              WRITE(KW(1),'(/T10,A,F6.3,A)')'**********HARGREAVES EQ USED TO EST &            
              POT ET HGX = ',HGX,'**********'                                                 
          CASE(5)                                                                      
      ...
      DST0=TX                                                                        
      CALL APAGE(0)                                                                  
      WRITE(KW(1),'(//1X,A,A/)')'____________________GENERAL INFORMATION__',&            
      ...
      IF(NSTP>0)THEN                                                                 
          ...
          IF(ISTP>0)THEN                                                               
              KRX=KR(22)
              ADUM='RTOPSC.DAT'                                                               
              CALL OPENV(KR(22),ADUM,0,KW(MSO))                                      
	      END IF                                                                            
	  ELSE                                                                                
      ...
      SELECT CASE(IOX)                                                                         
          ...
          CASE(2)    
              WRITE(KW(1),'(T10,A)')'O2=CGO2(ISL)/CGO2(LD1)'                                               
          CASE DEFAULT                                                                                
...
!     LINE 2/3                                                                       
      READ(KR(14),'(10F8.0)')SALB,HSG,FFC,WTMN,WTMX,WTBL,GWST,GWMX,RFT0,RFPK,&              
...
!                                               (BLANK IF UNKNOWN)                   
! 28  ST   = INITIAL SOIL WATER STORAGE (m/m)                      
...
!     LINES 4/47 
      FMT='(15F8.0)'                                                 
      READ(KR(14),FMT)(Z(I),I=1,MSL)
      READ(KR(14),FMT)(BD(I),I=1,MSL)
      READ(KR(14),FMT)(U(I),I=1,MSL)
      READ(KR(14),FMT)(FC(I),I=1,MSL)
      READ(KR(14),FMT)(SAN(I),I=1,MSL)
      READ(KR(14),FMT)(SIL(I),I=1,MSL)
      READ(KR(14),FMT)(WON(I),I=1,MSL)
      READ(KR(14),FMT)(PH(I),I=1,MSL)
      READ(KR(14),FMT)(SMB(I),I=1,MSL)
      READ(KR(14),FMT)(WOC(I),I=1,MSL)
      READ(KR(14),FMT)(CAC(I),I=1,MSL)
      READ(KR(14),FMT)(CEC(I),I=1,MSL)
      READ(KR(14),FMT)(ROK(I),I=1,MSL)
      READ(KR(14),FMT)(CNDS(I),I=1,MSL)
      READ(KR(14),FMT)(PKRZ(I),I=1,MSL)
      READ(KR(14),FMT)(RSD(I),I=1,MSL)
      READ(KR(14),FMT)(BDD(I),I=1,MSL)
      READ(KR(14),FMT)(PSP(I),I=1,MSL)
      READ(KR(14),FMT)(SATC(I),I=1,MSL)
      READ(KR(14),FMT)(HCL(I),I=1,MSL)
      READ(KR(14),FMT)(WP(I),I=1,MSL)
      READ(KR(14),FMT)(EXCK(I),I=1,MSL)
      READ(KR(14),FMT)(ECND(I),I=1,MSL)
      READ(KR(14),FMT)(STFR(I),I=1,MSL)
      READ(KR(14),FMT)(ST(I),I=1,MSL)
      READ(KR(14),FMT)(CPRV(I),I=1,MSL)
      READ(KR(14),FMT)(CPRH(I),I=1,MSL)
      READ(KR(14),FMT)(WLS(I),I=1,MSL)
      READ(KR(14),FMT)(WLM(I),I=1,MSL)
      READ(KR(14),FMT)(WLSL(I),I=1,MSL)
      READ(KR(14),FMT)(WLSC(I),I=1,MSL)
      READ(KR(14),FMT)(WLMC(I),I=1,MSL)
      READ(KR(14),FMT)(WLSLC(I),I=1,MSL)
      READ(KR(14),FMT)(WLSLNC(I),I=1,MSL)
      READ(KR(14),FMT)(WBMC(I),I=1,MSL)
      READ(KR(14),FMT)(WHSC(I),I=1,MSL)
      READ(KR(14),FMT)(WHPC(I),I=1,MSL)
      READ(KR(14),FMT)(WLSN(I),I=1,MSL)
      READ(KR(14),FMT)(WLMN(I),I=1,MSL)
      READ(KR(14),FMT)(WBMN(I),I=1,MSL)
      READ(KR(14),FMT)(WHSN(I),I=1,MSL)
      READ(KR(14),FMT)(WHPN(I),I=1,MSL)
      READ(KR(14),FMT)(FE26(I),I=1,MSL)
      READ(KR(14),FMT)(SULF(I),I=1,MSL)
      ! LINE 48      
      READ(KR(14),'(15A8)')ASHZ
      ! LINES 49/51      
      IF(IDN>2)THEN
          READ(KR(14),FMT)(CGO2(J),J=1,MSL)
          READ(KR(14),FMT)(CGCO2(J),J=1,MSL)
          READ(KR(14),FMT)(CGN2O(J),J=1,MSL)
      ELSE
          CGO2=0.
          CGCO2=0.
          CGN2O=0.
      ...
      DO J=1,MSL ! SOIL LOOP                                                                
          ...
          SELECT CASE(ISW+1)                                                             
	          ...
	          CASE(7)
                  IF(U(J)<1.E-10.OR.FC(J)<1.E-10)THEN
                      CEM(J)=MAX(.1,CEC(J)-2.428*X2-1.7*ZZ)
                      CALL SWRTNB(CEM(J),CLA(J),X2,SAN(J),U(J),FC(J),ZZ)
                  END IF                           
	          CASE(8,9)                                                                         
	          ...
	          CASE(10,11)                                                                         
                  CALL SWRTNNW(CLA(J),SIL(J),SAN(J),X2,BD(J),U(J),FC(J))
              CASE DEFAULT
                  IF(ISW/=2.AND.ISW/=3)CALL SWRTNR(CLA(J),SAN(J),X2,U(J),FC(J))
                  ISW=0   
          END SELECT
          ...
          VGA(J)=EXP(-4.3003-.0097*CLA(J)+.0138*SAN(J)-.1706*X2)
          VGN0=1.
          VGA0=VGA(J)
          U0=S15(J)
          PO0=PO(J)
          FC0=FC(J)
          DO IT=1,10
              FU=U0+(PO0-U0)/(1.+(VGA0*336.27)**VGN0)**(1.-1./VGN0)-FC0
      ...
      DO J=1,NBSL                                                                
	      ...
	      !IF(L==1)THEN
	          !L1=LID(L+1)
	          !WHPC(L1)=WHPC(L1)+WHPC(L)
	          !WHPN(L1)=WHPN(L1)+WHPN(L)
	          !WHPC(L)=0.
	          !WHPN(L)=0.
	      !END IF
          XZP(1,L)=WHSC(L)                                                               
...
!     OCPD=.1*OCPD/SUM                                                               
      ...
      WRITE(KW(1),'(T10,A,A20)')'SOIL SERIES = ',SOLS                                  
      WRITE(KW(1),'(T10,A,A20)')'SOIL ORDER = ',SOLO                                  
      ...
      SELECT CASE(ISW+1)                                                             
	      ...
	      CASE(9)                                                                           
              ...
              STATIC'                                                            
          CASE(10)                                                                           
	          WRITE(KW(1),'(T10,A)')'FC/WP EST BNW METHOD DYNAMIC'
          CASE(11)                                                                           
	          WRITE(KW(1),'(T10,A)')'FC/WP EST BNW METHOD STATIC'	              
	  END SELECT
	  IF(ISAT>0)THEN
...
!     LINE 2                                                                         
      ...
      IF(IWRT==0)THEN
          ...
          DO I=2,MSO                                                                 
              ...
              IF(I==9)THEN
                  XCC=1.                                                                         
                  X1=0.                                                                          
                  WRITE(KW(9),523)SOLS,SOLO,IYER,IMON,IDAY                                        
                  WRITE(KW(9),303)SALB,HSG,FFC,WTMN,WTMX,WTBL,GWST,GWMX,RFT0,RFPK,&              
                  TSLA,XIDP,X1,XIDK,ZQT,ZF,ZTK,FBM,FHP,XCC                                       
                  CYCLE
              END IF                                                                      
      ...
      IF(KFL(17)>0)WRITE(KW(17),826)(HED(KD(J1)),J1=1,NKD),'ZNH3','ZNO3',&
      'NO31','PRK1','LN31',' ALB',' HUI','AJHI',' LAI','BIOM','  RW',&
      ' STL','  HI','YLDX','YLDF','UNO3',' NPP',' NEE',(HEDS(I),I=23,28)                                                                       
      IF(KFL(18)>0)THEN
          IF(NGN>0)THEN
              A1='INPUT WEATHER'
              A2=FWTH
          ELSE
              ...
              A2=' '
          END IF
          WRITE(KW(18),671)ASTN,YLAT,YLAZ,UPS,ZCS,A1,A2                              
      END IF
	  ...
	  END IF                                                                              
      WRITE(KW(1),'(//1X,A/)')'____________________MANAGEMENT DATA______&            
      _______________'                                                               
  ...
  142 IF(BFT0>0.)THEN
          ...
          IF(BFT0>1.)THEN                                                                
              ...
              ' g/t'                                                                       
          ELSE                                                                           
              WRITE(KW(1),'(T15,A,F6.2)')'PLANT STRESS TRIGGER = ',BFT0                    
          END IF                                                                         
          IF(FNP>1.)THEN                                                                 
      ...
      ELSE
          IAUF=0                                                                         
          IF(MNU==0)WRITE(KW(1),'(T10,A)')'USER SCHEDULED FERT'                                    
      END IF    
  433 WRITE(KW(1),'(T15,A,F7.0,A)')'MAX N FERT/CROP = ',FMX,' kg/ha'
      ...
      END IF                                                                     
      IF(IFD>0)WRITE(KW(1),'(T10,A,F5.2)')'FURROW DIKE SYSTEM SAFETY &
      FACTOR = ',FDSF
      IF(IMW>0)WRITE(KW(1),'(T10,A,I4,A)')'AUTO MOW INTERVAL = ',I4,' d'
      WRITE(KW(1),'(T10,A,F6.3)')'USLE P FACTOR = ',PEC                              
      RFTT=1.-EXP(-1./RFTT)                                                          
      FDSF=FDSF*1000.                                                                
      IF(LMS==0)THEN                                                                 
...
!     PRINTOUT CROP PARAMETERS                                                       
      ...
      WRITE(KW(1),78)'CCEM',(CCEM(I),I=1,LC)
      WRITE(KW(1),78)'FLSL',(FLSL(I),I=1,LC)
      ...
      IF(NDP>0)THEN
          WRITE(KW(1),'(//1X,A/)')'____________________PESTICIDE DATA_______&            
          _____________'                                                                 
          WRITE(KW(1),382)                                                               
  ...
  678 XLC=LC
      ...
      DO J=1,LC
          ...
          STD(J)=X1
      END DO
      IF(RZ>XX)RZ=XX    ! Root zone set to max soil depth (xx) if max root depth > than xx
      IF(BIG>XX)BIG=XX                                                               
      BTN=TWN+ZNO3+SUM                                                              
      BTNX=BTN                                                                            
      BTC=TOC
...
!     BEGIN ANNUAL SIMULATION LOOP                                                   
  ...
  533 CALL BSIM                                                                      
      ...
      IF(KFL(9)>0)THEN
          ! LINES 4/51                                                                     
          WRITE(KW(9),625)(Z(LID(I)),I=1,MSL),'  4 DEPTH(m)        '
          WRITE(KW(9),625)(BD(LID(I)),I=1,MSL),'  5 BD 33kpa(t/m3)  '                                   
          WRITE(KW(9),625)(SOIL(20,LID(I)),I=1,MSL),'  6 WP SW(m/m)       '                                       
          WRITE(KW(9),625)(SOIL(9,LID(I)),I=1,MSL),'  7 FC SW(m/m)      '                                   
          WRITE(KW(9),625)(SAN(LID(I)),I=1,MSL),'  8 SAND(%)         '                                  
          WRITE(KW(9),625)(SIL(LID(I)),I=1,MSL),'  9 SILT(%)         '                                   
          WRITE(KW(9),695)(SOIL(6,LID(I)),I=1,MSL),' 10 ORG N(g/t)      '                                   
          WRITE(KW(9),625)(PH(LID(I)),I=1,MSL),' 11 PH              '                                   
          WRITE(KW(9),625)(SMB(LID(I)),I=1,MSL),' 12 SM BS(cmol/kg)  '                                   
          WRITE(KW(9),625)(SOIL(7,LID(I)),I=1,MSL),' 13 ORG C(%)        '                                   
          WRITE(KW(9),625)(CAC(LID(I)),I=1,MSL),' 14 CAC(%)          '                                   
          WRITE(KW(9),625)(CEC(LID(I)),I=1,MSL),' 15 CEC(cmol/kg)    '                                   
          WRITE(KW(9),625)(ROK(LID(I)),I=1,MSL),' 16 ROCK(%)         '                                   
          WRITE(KW(9),625)(SOIL(5,LID(I)),I=1,MSL),' 17 NO3(g/t)        '                                   
          WRITE(KW(9),625)(SOIL(1,LID(I)),I=1,MSL),' 18 LAB P(g/t)      '                                   
          WRITE(KW(9),625)(RSD(LID(I)),I=1,MSL),' 19 CROP RSD(t/ha)  '                                   
          WRITE(KW(9),625)(SOIL(13,LID(I)),I=1,MSL),' 20 BD DRY(t/m3)    '                                   
          WRITE(KW(9),625)(PSP(LID(I)),I=1,MSL),' 21 P SORP RTO       '                                   
          WRITE(KW(9),625)(SATC(LID(I)),I=1,MSL),' 22 SAT COND(mm/h)   '                                   
	      WRITE(KW(9),625)(HCL(LID(I)),I=1,MSL),' 23 H SC(mm/h)       '                                        
          WRITE(KW(9),625)(SOIL(4,LID(I)),I=1,MSL),' 24 ORG P(g/t)       '                                   
          WRITE(KW(9),625)(SOIL(15,LID(I)),I=1,MSL),' 25 EXCK(g/t)        '                                   
          WRITE(KW(9),625)(ECND(LID(I)),I=1,MSL),' 26 ECND(mmho/cm)   '                                   
	      WRITE(KW(9),625)(STFR(LID(I)),I=1,MSL),' 27 STFR            '                                        
          WRITE(KW(9),625)(SOIL(12,LID(I)),I=1,MSL),' 28 SW(m/m)         '                                   
          WRITE(KW(9),695)(CPRV(LID(I)),I=1,MSL),' 29 FRACT V PIPE F  '                                        
	      WRITE(KW(9),695)(CPRH(LID(I)),I=1,MSL),' 30 FRACT H PIPE F  '                                        
          WRITE(KW(9),695)(WLS(LID(I)),I=1,MSL),' 31 WLS(kg/ha)      '                                   
          WRITE(KW(9),695)(WLM(LID(I)),I=1,MSL),' 32 WLM(kg/ha)      '                                   
          WRITE(KW(9),625)(WLSL(LID(I)),I=1,MSL),' 33 WLSL(kg/ha)     '                                   
          WRITE(KW(9),625)(WLSC(LID(I)),I=1,MSL),' 34 WLSC(kg/ha)     '                                   
          WRITE(KW(9),625)(WLMC(LID(I)),I=1,MSL),' 35 WLMC(kg/ha)     '                                   
          WRITE(KW(9),625)(WLSLC(LID(I)),I=1,MSL),' 36 WLSLC(kg/ha)    '                                   
          WRITE(KW(9),625)(WLSLNC(LID(I)),I=1,MSL),' 37 WLSLNC(kg/ha)   '                                   
          WRITE(KW(9),625)(WBMC(LID(I)),I=1,MSL),' 38 WBMC(kg/ha)     '                                   
          WRITE(KW(9),695)(WHSC(LID(I)),I=1,MSL),' 39 WHSC(kg/ha)     '                                   
          WRITE(KW(9),695)(WHPC(LID(I)),I=1,MSL),' 40 WHPC(kg/ha)     '                                   
          WRITE(KW(9),625)(WLSN(LID(I)),I=1,MSL),' 41 WLSN(kg/ha)     '                                   
          WRITE(KW(9),625)(WLMN(LID(I)),I=1,MSL),' 42 WLMN(kg/ha)     '                                   
          WRITE(KW(9),625)(WBMN(LID(I)),I=1,MSL),' 43 WBMN(kg/ha)     '                                   
          WRITE(KW(9),695)(WHSN(LID(I)),I=1,MSL),' 44 WHSN(kg/ha)     '                                   
          WRITE(KW(9),695)(WHPN(LID(I)),I=1,MSL),' 45 WHPN(kg/ha)     '
          WRITE(KW(9),625)(FE26(LID(I)),I=1,MSL),' 46 IRON(%)         '
          WRITE(KW(9),625)(SULF(LID(I)),I=1,MSL),' 47 SULPHUR(%)      '
          WRITE(KW(9),'(15A8,A20)')(ASHZ(LID(I)),I=1,MSL),' 48 SOIL HORIZON    '
          WRITE(KW(9),625)(CGO2(LID(I)),I=1,MSL),' 49 CGO2(kg/ha)     '
          WRITE(KW(9),625)(CGCO2(LID(I)),I=1,MSL),' 50 CGCO2(kg/ha)    '
          WRITE(KW(9),625)(CGN2O(LID(I)),I=1,MSL),' 51 CGN2O(kg/ha)    '
      END IF
      WRITE(KW(1),'(//1X,A/)')'____________________FINAL SOIL DATA______&            
      _______________'                                                               
      CALL SOLIO(YTP,1)                                                              
	  CALL SCONT(1)                                                                          
      WRITE(KW(1),'(/T10,A,F7.1,A)')'ERODED SOIL THICKNESS = ',THK,' mm'             
      WRITE(KW(1),'(/T10,A,F7.2,A)')'FINAL WATER CONTENT OF SNOW = ',SNO&            
      ,' mm'                                                                         
...
!     DETERMINE AVE ANNUAL VALUES                                                    
      ...
      CALL APAGE(1)                                                                  
      WRITE(KW(1),'(//1X,A/)')'______________PESTICIDE SUMMARY TABLE____&            
      ____________'                                                                  
      WRITE(KW(1),460)                                                               
      NY(1)=1                                                                        
      NY(2)=.1*XYR+1.5                                                               
...
!     PRINTOUT PESTICIDE SUMMARY                                                     
  ...
  212 PPX(1)=SM(44)                                                                  
      ...
      WRITE(KW(1),'(/1X,A)')'-----SOIL LAYER (ST-FC)/(PO-FC)'
      ...
      WRITE(KW(1),731)(Z(LID(K)),SLTP(LID(K)),K=1,NBSL)
...
!  4  JZ(4) = DAILY WEATHER STA # FROM KR(27) WTHCOM.DAT                             
  ...
  233 FORMAT(T10,'PEAK RATE-EI ADJ FACTOR = ',F6.3/T10,'INITIAL WATER CO&            
      NTENT OF SNOW = ',F5.1,' mm'/T10,'AVE N CONC IN RAINFALL = ',F6.2,&            
  ...
  234 FORMAT(/1X,'-----CURVE NUMBER DISTRIBUTION'/T10,'>95=',F6.2,3X,'>&             
      90=',F6.2,3X,'>85=',F6.2,3X,'>80=',F6.2,3X,'>75=',F6.2,3X,'>70=',&             
      F6.2,3X,'>65=',F6.2,3X,'>60=',F6.2,3X,'>55=',F6.2,3X,'<55=',F6.2)              
  ...
  285 FORMAT(T10,'FIELD LENGTH = ',F6.2,' km'/T10,'FIELD WIDTH = ',&                 
      ...
      P POWER PRMT = ',F6.2/T10,'SOIL PARTICLE DIAM = ',F5.0,' UM'/T10,&             
      'ACCELERATE WIND EROS FACTOR = ',F7.3)                                         
  286 FORMAT(5X,3I4)                                                                 
  287 FORMAT(20(1X,A4))                                                              
  290 FORMAT(T10,'SOIL ALBEDO = ',F6.2/T10,'MAX NUMBER SOIL LAYERS = ',&             
      F3.0/T10,'MIN THICKNESS FOR LAYER SPLITTING = ',F6.2,' m'/T10,'MIN &            
       PROFILE THICKNESS--STOPS SIMULATION = ',F6.2,' m'/T10,'SPLITTING'&            
      ,' PRIORITY THICKNESS = ',F6.2,' m'/T10,'BIOMASS/ORG C = ',F6.3/&              
      T10,'PASSIVE HUMUS/TOTAL HUMUS = ',F6.3/T10,'CULTIVATION HISTORY =&            
       ',F5.0,' y'/T10,'WEATHERING CODE = ',F3.0/T10,'SOIL GROUP CODE ='&            
      ,1X,F3.0/T10,'ORG C IN TOP ',F6.2,' m = ',F7.1,' t/ha')                        
  ...
  316 FORMAT(/T10,'TILLAGE OPERATIONS'/6X,'DATE',T21,'OP',4X,'EQ',4X,&               
      ...
      ,'VIRR',2X,'HUSC')                                                             
  317 FORMAT(4X,I3,2I2,1X,A8,I2,2I6,2F7.2,7X,F6.2,2F6.0,F6.3,F6.0,F6.2,F6.0&            
  ...
  323 FORMAT(/1X,'-----PEAK FLOW RATE STATS(mm/h)',T70,'UNIT PEAKS(1/h)&             
      ...
      'MAX = ',F7.4,5X,'MEAN = ',F7.4,5X,'NO PKS = ',I6)                             
  ...
  351 FORMAT(T10,'WATER TABLE DEPTH'/T15,'MIN = ',F6.2,' m'/T15,'MAX = '&            
      ...
      ,' FLOW TT = ',F7.2,' d')                                                      
  354 FORMAT(T15,'MAX ANNUAL VOL APPL TO A CROP = ',F6.0,' mm'/T15,'MIN &             
  ...
  365 FORMAT(T10,'DRAINAGE AREA = ',F9.2,' ha'/T10,'LATITUDE = ',F7.2,' &             
      ...
      T10,'CHANNEL'/T15,'LENGTH = ',F6.2,' km',5X,'GRAD = ',F7.4,' m/m',&            
      5X,'MANNINGS N = ',F6.3,5X,'DEPTH = ',F6.3,' m'/T10,'LAND SLOPE'/&             
  ...
  396 FORMAT(8F10.6)                                                                 
  405 FORMAT(T15,'LS= ',F6.3/T10,'RUSLE XM = ',F6.3/T10,'RUSLE SLP LG FA&            
      C = ',F6.3/T10,'RUSLE SLP FAC = ',F6.3/T10,'TIME OF FLOW CONC = ',&            
      F5.2,' h')                                                                     
  ...
  445 FORMAT(T15,'BIG GUN IRRIGATION FROM LAGOON'/T15,'CONFINEMENT AREA &             
      ...
      WATER = ',F6.3,' m3/cow/d')                                                  
  448 FORMAT(/1X,'-----SEDIMENT CONCENTRATION STATS(g/m3)'/T10,'MEAN &             
  ...
  551 FORMAT(1X,' YR  RT#',1X,'CPNM',3X,'YLDG',4X,'YLDF',4X,'WCYD',4X,&
      ...
      ' CAW',4X,' CRF',4X,' CQV',4X,'COST',4X,'COOP',4X,'RYLG',4X,'RYLF',&
      4X,'PSTF',4X,'  WS',4X,'  NS',4X,'  PS',4X,'  KS',4X,'  TS',4X,&
      '  AS',4X,'  SS',4X,'PPOP',5X,'IPLD',5X,'IGMD',5X,'IHVD ')                                     
  557 FORMAT(1X,' YR  RT#',24(4X,A4),4X,'OCPD',4X,' TOC',4X,'APBC',4X,&              
     &' TAP',4X,'ZNO3')                                                              
  558 FORMAT(1X,'RUN   YR',4X,'AP15',4X,'PMTE',41(3X,A4,1X))                                   
  ...
  602 FORMAT(19X,'WPM1 = ',A12/T11,'JAN',6X,'FEB',6X,'MAR',6X,'APR',6X,&             
      ...
      'DEC',6X,' YR')                                                                
  607 FORMAT(2X,I4)                                                                  
  621 FORMAT(/T5,'EPIC1102 v2020-01-23',2X,3I4,2X,2(I2,':'),I2/)                               
  ...
  716 FORMAT(T10,'AUTO FERT EQUIP = ',A8,2X,'DEPTH = ',F6.3,' M',2X,&                
      'FERT = ',A8/)                                                                 
  722 FORMAT(T55,'RWT SOIL LAYER #'/3X,'Y',3X,'Y#','   M   D',1X,'CROP',&
      4X,'BIOM',16(4X,A4))                                                              
  723 FORMAT(10X,A15,5X,16F8.2)                                                      
